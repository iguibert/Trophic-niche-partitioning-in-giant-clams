#######################################################################################
## HERS score: Exemple of script to run on real dataset                                ####
## Data: giant clams isotope data - Bakers Lab - University of Hong Kong - SWIRE       ####
## Leonard Pons (Not optimized)                                                        ####
#######################################################################################
# Packages #####
...
# R doesn't provide a statistical mode function, use the following code:
...
Mode <- function(x){
  ...
  most.occur.ind <- c(unique(most.occur.ind), rep(NA, times = 5-length(unique(most.occur.ind))))
  ...
  count.most.occur.val <- c(length(uni.vect[count.values == most.occur.ind[1]]),
                            length(uni.vect[count.values == most.occur.ind[2]]),
                            length(uni.vect[count.values == most.occur.ind[3]]),
                            length(uni.vect[count.values == most.occur.ind[4]]),
                            length(uni.vect[count.values == most.occur.ind[5]]))
  
  weight <- rep(most.occur.ind,count.most.occur.val)
  weighted.mean(x = most.occur.val, w = weight, na.rm = TRUE)
}
...
for (i in 1:length(Yourdataset_splited) #For all data frame created (1 per specie)
  ...
  # Create objects which will be used in the script
  Master_list[[i]] <- list(
    Species = c(), 
  ...
  # Artificially create a community metric for SIBER
  df_siber$Species <- 1
  # be sure your (Host/Symbiont) column is named => TYPE
  df_siber$Tissues[df_siber$Tissues == "H"] <- 1
  df_siber$Tissues[df_siber$Tissues == "S"] <- 2
  # Rename all columns to be SIBER friendly
  ...
  for (it in 1:iteration) {
    ...
    # FROM HERE IT'S JUST SIBER ANALYSIS
...
saveWorkbook(NewFile, file = "GiantClamsHERSResults202510.xlsx")
